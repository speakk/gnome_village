...
pub fn attach_to_settler(
    query: Query<Entity, (Added<Settler>, With<InWorld>, Without<Prototype>)>,
    ...
    for entity in query.iter() {
        let action = commands
            ...
            .with_children(|root| {
                root.spawn((StuckInWallScorer, EscapeFromSolidAction));
            })
            .id();

        commands
            .entity(action)
            .trigger(OnRunAction::new(action, entity, ()));
    }
}
...
fn provide_score(
    ...
    let world_position = world_positions
        .get(target_entity)
        .expect("WorldPosition not found for target entity");
