...
fn get_available_task(task_entity: Entity,
                      task_data: &Task,
                      children: Option<&Children>,
                      all_tasks_query: &Query<(Entity, &Task, Option<&Children>)>) -> Option<Entity> {
        ...
        RunType::Sequence => {
            if let Some(children) = children {
                for &child in children.iter() {
                    let (_entity, child_task_data, sub_children) = all_tasks_query.get(child).unwrap();
                    let next_sub_task = get_available_task(child, task_data, sub_children, all_tasks_query);
                    if let Some(next_sub_task) = next_sub_task {
                        let (_, next_sub_task_data, _) = all_tasks_query.get(next_sub_task).unwrap();
