...
func _init(smelter_component: SmelterComponent, recipe: Recipe) -> void:
	task_name = "Smelt ore"
	name = "SmeltTree"
	order_type = Task.OrderType.Sequence
	
	var material_requirements := recipe.requires
		...
		for material_requirement in material_requirements as Array[ItemRequirement]:
			for i in material_requirement.amount:
				...
				var task := BringResourceTask.new({
					...
					inventory_component = smelter_component.get_inventory()
				})
	...
	var smelt_task := SmeltTask.new(smelter_component)
	register_subtask(smelt_task)
